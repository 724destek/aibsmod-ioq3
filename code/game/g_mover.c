...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
		...
		if ( check->s.eType != ET_ITEM && check->s.eType != ET_PLAYER && check->s.eType != ET_CLONE && !check->physicsObject ) {
			continue;
		}
		...
		*obstacle = check;
		...
		for ( p=pushed_p-1 ; p>=pushed ; p-- ) {
			...
			//aibsmod - the above should be enough for clones
			if ((p->ent->s.eType != ET_CLONE) && ( p->ent->client )) {
