...
static void CG_DrawPlayerArmorIcon( rectDef_t *rect, qboolean draw2D ) {
	...
	if ( draw2D || ( !cg_draw3dIcons.integer && cg_drawIcons.integer) ) {
		...
		CG_Draw3DModel( rect->x, rect->y, rect->w, rect->h, cgs.media.armorModel, 0, origin, angles, -1, AM_COLORPART_NONE );
	}
}
...
static void CG_DrawPlayerAmmoIcon( rectDef_t *rect, qboolean draw2D ) {
	...
	if ( draw2D || (!cg_draw3dIcons.integer && cg_drawIcons.integer) ) {
		...
		if ( cent->currentState.weapon && cg_weapons[ cent->currentState.weapon ].ammoModel ) {
			...
			CG_Draw3DModel( rect->x, rect->y, rect->w, rect->h, cg_weapons[ cent->currentState.weapon ].ammoModel, 0, origin, angles, -1, AM_COLORPART_NONE );
		}
	}
}
	...
	if ( value ) {
		...
		if (qtrue) {
  		...
  		angles[YAW] = ( cg.time & 2047 ) * 360 / 2048.0;
			CG_Draw3DModel(rect->x, rect->y, rect->w, rect->h, cg_items[ value ].models[0], 0, origin, angles, -1, AM_COLORPART_NONE );
		}
	}
...
static void CG_DrawSelectedPlayerHead( rectDef_t *rect, qboolean draw2D, qboolean voice ) {
  ...
  if (ci) {
      ...
      CG_Draw3DModel( rect->x, rect->y, rect->w, rect->h, ci->headModel, ci->headSkin, origin, angles, clientNum, AM_COLORPART_NONE );
...
static void CG_HarvesterSkulls(rectDef_t *rect, float scale, vec4_t color, qboolean force2D, int textStyle ) {
	...
	if (cg_drawIcons.integer) {
		if (!force2D && cg_draw3dIcons.integer) {
			...
			CG_Draw3DModel( rect->x, rect->y, 35, 35, handle, 0, origin, angles, -1, AM_COLORPART_NONE );
		} else {
			if( cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE ) {
